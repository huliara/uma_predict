from uma_predict.db.database import Reflected, Base, engine
import uuid
from sqlalchemy.orm import Mapped, mapped_column, relationship
from datetime import date


class Race(Reflected, Base):
    __tablename__ = "jvd_ra"
    record_id: Mapped[str]
    data_kubun: Mapped[str]
    data_sakusei_nengappi: Mapped[str]
    kaisai_nen: Mapped[str] = mapped_column("kaisai_nen", primary_key=True)
    kaisai_tsukihi: Mapped[str] = mapped_column(
        "kaisai_tsukihi", primary_key=True
    )
    keibajo_code: Mapped[str] = mapped_column("keibajo_code", primary_key=True)
    kaisai_kai: Mapped[str]
    kaisai_nichime: Mapped[str]
    race_bango: Mapped[str] = mapped_column("race_bango", primary_key=True)
    grade_code: Mapped[str]
    grade_code_henkomae: Mapped[str]
    kyoso_shubetsu_code: Mapped[str]
    kyoso_kigo_code: Mapped[str]
    juryo_shubetsu_code: Mapped[str]
    kyoso_joken_code_2sai: Mapped[str]
    kyoso_joken_code_3sai: Mapped[str]
    kyoso_joken_code_4sai: Mapped[str]
    kyoso_joken_code_5sai_ijo: Mapped[str]
    kyoso_joken_code: Mapped[str]
    kyoso_joken_meisho: Mapped[str]
    kyori: Mapped[str]
    kyori_henkomae: Mapped[str]
    track_code: Mapped[str]
    track_code_henkomae: Mapped[str]
    course_kubun: Mapped[str]
    course_kubun_henkomae: Mapped[str]
    honshokin: Mapped[str]
    honshokin_henkomae: Mapped[str]
    fukashokin: Mapped[str]
    fukashokin_henkomae: Mapped[str]
    hasso_jikoku: Mapped[str]
    hasso_jikoku_henkomae: Mapped[str]
    toroku_tosu: Mapped[str]
    shusso_tosu: Mapped[str]
    nyusen_tosu: Mapped[str]
    tenko_code: Mapped[str]
    babajotai_code_shiba: Mapped[str]
    babajotai_code_dirt: Mapped[str]
    lap_time: Mapped[str]
    shogai_mile_time: Mapped[str]
    zenhan_3f: Mapped[str]
    zenhan_4f: Mapped[str]
    kohan_3f: Mapped[str]
    kohan_4f: Mapped[str]
    corner_tsuka_juni_1: Mapped[str]
    corner_tsuka_juni_2: Mapped[str]
    corner_tsuka_juni_3: Mapped[str]
    corner_tsuka_juni_4: Mapped[str]
    record_koshin_kubun: Mapped[str]


class Horse(Reflected, Base):
    __tablename__ = "jvd_um"
    record_id: Mapped[str]
    data_kubun: Mapped[str]
    data_sakusei_nengappi: Mapped[str]
    ketto_toroku_bango: Mapped[str] = mapped_column(
        "ketto_toroku_bango", primary_key=True
    )
    toroku_nengappi: Mapped[str]
    seinengappi: Mapped[str]
    bamei: Mapped[str]
    bamei_hankaku_kana: Mapped[str]
    bamei_eur: Mapped[str]
    zaikyu_flag: Mapped[str]
    umakigo_code: Mapped[str]
    seibetsu_code: Mapped[str]
    hinshu_code: Mapped[str]
    moshoku_code: Mapped[str]
    ketto_joho_01a: Mapped[str]
    ketto_joho_01b: Mapped[str]
    ketto_joho_02a: Mapped[str]
    ketto_joho_02b: Mapped[str]
    ketto_joho_03a: Mapped[str]
    ketto_joho_03b: Mapped[str]
    ketto_joho_04a: Mapped[str]
    ketto_joho_04b: Mapped[str]
    ketto_joho_05a: Mapped[str]
    ketto_joho_05b: Mapped[str]
    ketto_joho_06a: Mapped[str]
    ketto_joho_06b: Mapped[str]
    ketto_joho_07a: Mapped[str]
    ketto_joho_07b: Mapped[str]
    ketto_joho_08a: Mapped[str]
    ketto_joho_08b: Mapped[str]
    ketto_joho_09a: Mapped[str]
    ketto_joho_09b: Mapped[str]
    ketto_joho_10a: Mapped[str]
    ketto_joho_10b: Mapped[str]
    ketto_joho_11a: Mapped[str]
    ketto_joho_11b: Mapped[str]
    ketto_joho_12a: Mapped[str]
    ketto_joho_12b: Mapped[str]
    ketto_joho_13a: Mapped[str]
    ketto_joho_13b: Mapped[str]
    ketto_joho_14a: Mapped[str]
    ketto_joho_14b: Mapped[str]
    tozai_shozoku_code: Mapped[str]
    chokyoshi_code: Mapped[str]
    chokyoshimei_ryakusho: Mapped[str]
    heichi_honshokin_ruikei: Mapped[str]
    shogai_honshokin_ruikei: Mapped[str]
    heichi_fukashokin_ruikei: Mapped[str]
    shogai_fukashokin_ruikei: Mapped[str]
    heichi_shutokushokin_ruikei: Mapped[str]
    shogai_shutokushokin_ruikei: Mapped[str]
    sogo: Mapped[str]
    chuo_gokei: Mapped[str]
    shiba_choku: Mapped[str]
    shiba_migi: Mapped[str]
    shiba_hidari: Mapped[str]
    dirt_choku: Mapped[str]
    dirt_migi: Mapped[str]
    dirt_hidari: Mapped[str]
    shogai: Mapped[str]
    shiba_ryo: Mapped[str]
    shiba_yayaomo: Mapped[str]
    shiba_omo: Mapped[str]
    shiba_furyo: Mapped[str]
    dirt_ryo: Mapped[str]
    dirt_yayaomo: Mapped[str]
    dirt_omo: Mapped[str]
    dirt_furyo: Mapped[str]
    shogai_ryo: Mapped[str]
    shogai_yayaomo: Mapped[str]
    shogai_omo: Mapped[str]
    shogai_furyo: Mapped[str]
    shiba_short: Mapped[str]
    shiba_middle: Mapped[str]
    shiba_long: Mapped[str]
    dirt_short: Mapped[str]
    dirt_middle: Mapped[str]
    dirt_long: Mapped[str]
    kyakushitsu_keiko: Mapped[str]
    toroku_race_su: Mapped[str]


class Career(Reflected, Base):
    __tablename__ = "jvd_se"
    record_id                   :Mapped[str]
    data_kubun                  :Mapped[str]
    data_sakusei_nengappi       :Mapped[str]
    kaisai_nen                  :Mapped[str]=mapped_column("kaisai_nen", primary_key=True)
    kaisai_tsukihi              :Mapped[str]=mapped_column("kaisai_tsukihi", primary_key=True)
    keibajo_code                :Mapped[str]=mapped_column("keibajo_code", primary_key=True)
    kaisai_kai                  :Mapped[str]
    kaisai_nichime              :Mapped[str]
    race_bango                  :Mapped[str]=mapped_column("race_bango", primary_key=True)
    wakuban                     :Mapped[str]
    umaban                      :Mapped[str]=mapped_column("umaban", primary_key=True)
    ketto_toroku_bango          :Mapped[str]=mapped_column("ketto_toroku_bango", primary_key=True)
    bamei                       :Mapped[str]
    umakigo_code                :Mapped[str]
    seibetsu_code               :Mapped[str]
    hinshu_code                 :Mapped[str]
    moshoku_code                :Mapped[str]
    barei                       :Mapped[str]
    tozai_shozoku_code          :Mapped[str]
    chokyoshi_code              :Mapped[str]
    chokyoshimei_ryakusho       :Mapped[str]
    futan_juryo                 :Mapped[str]
    futan_juryo_henkomae        :Mapped[str]
    blinker_shiyo_kubun         :Mapped[str]
    kishu_code                  :Mapped[str]
    kishu_code_henkomae         :Mapped[str]
    kishumei_ryakusho           :Mapped[str]
    kishumei_ryakusho_henkomae  :Mapped[str]
    kishu_minarai_code          :Mapped[str]
    kishu_minarai_code_henkomae :Mapped[str]
    bataiju                     :Mapped[str]
    zogen_fugo                  :Mapped[str]
    zogen_sa                    :Mapped[str]
    ijo_kubun_code              :Mapped[str]
    nyusen_juni                 :Mapped[str]
    kakutei_chakujun            :Mapped[str]
    dochaku_kubun               :Mapped[str]
    dochaku_tosu                :Mapped[str]
    soha_time                   :Mapped[str]
    chakusa_code_1              :Mapped[str]
    chakusa_code_2              :Mapped[str]
    chakusa_code_3              :Mapped[str]
    corner_1                    :Mapped[str]
    corner_2                    :Mapped[str]
    corner_3                    :Mapped[str]
    corner_4                    :Mapped[str]
    tansho_odds                 :Mapped[str]
    tansho_ninkijun             :Mapped[str]
    kakutoku_honshokin          :Mapped[str]
    kakutoku_fukashokin         :Mapped[str]
    kohan_4f                    :Mapped[str]
    kohan_3f                    :Mapped[str]
    aiteuma_joho_1              :Mapped[str]
    aiteuma_joho_2              :Mapped[str]
    aiteuma_joho_3              :Mapped[str]
    time_sa                     :Mapped[str]
    record_koshin_kubun         :Mapped[str]
    mining_kubun                :Mapped[str]
    yoso_soha_time              :Mapped[str]
    yoso_gosa_plus              :Mapped[str]
    yoso_gosa_minus             :Mapped[str]
    yoso_juni                   :Mapped[str]
    kyakushitsu_hantei          :Mapped[str]

class Tansho(Reflected, Base):
    __tablename__ = "jvd_o1"
    record_id              :Mapped[str]               
    data_kubun             :Mapped[str]               
    data_sakusei_nengappi  :Mapped[str]               
    kaisai_nen             :Mapped[str] = mapped_column("kaisai_nen",primary_key=True)      
    kaisai_tsukihi         :Mapped[str] = mapped_column("kaisai_tsukihi",primary_key=True)      
    keibajo_code           :Mapped[str] = mapped_column("keibajo_code",primary_key=True)      
    kaisai_kai             :Mapped[str]               
    kaisai_nichime         :Mapped[str]               
    race_bango             :Mapped[str] = mapped_column("race_bango",primary_key=True)      
    happyo_tsukihi_jifun   :Mapped[str]               
    toroku_tosu            :Mapped[str]               
    shusso_tosu            :Mapped[str]               
    hatsubai_flag_tansho   :Mapped[str]               
    hatsubai_flag_fukusho  :Mapped[str]               
    hatsubai_flag_wakuren  :Mapped[str]               
    fukusho_chakubarai_key :Mapped[str]               
    odds_tansho            :Mapped[str]               
    odds_fukusho           :Mapped[str]               
    odds_wakuren           :Mapped[str]               
    hyosu_gokei_tansho     :Mapped[str]               
    hyosu_gokei_fukusho    :Mapped[str]               
    hyosu_gokei_wakuren    :Mapped[str]              


class Umaren(Reflected, Base):
    __tablename__ = "jvd_o2"
    record_id             :Mapped[str]              
    data_kubun            :Mapped[str]              
    data_sakusei_nengappi :Mapped[str]              
    kaisai_nen            :Mapped[str]=mapped_column("kaisai_nen",primary_key=True)      
    kaisai_tsukihi        :Mapped[str]=mapped_column("kaisai_tsukihi",primary_key=True)      
    keibajo_code          :Mapped[str]=mapped_column("keibajo_code",primary_key=True)      
    kaisai_kai            :Mapped[str]              
    kaisai_nichime        :Mapped[str]              
    race_bango            :Mapped[str]=mapped_column("race_bango",primary_key=True)      
    happyo_tsukihi_jifun  :Mapped[str]              
    toroku_tosu           :Mapped[str]              
    shusso_tosu           :Mapped[str]              
    hatsubai_flag_umaren  :Mapped[str]              
    odds_umaren           :Mapped[str]              
    hyosu_gokei_umaren    :Mapped[str]              

class Wide(Reflected, Base):
    __tablename__ = "jvd_o3"
    record_id             :Mapped[str]              
    data_kubun            :Mapped[str]              
    data_sakusei_nengappi :Mapped[str]              
    kaisai_nen            :Mapped[str]=mapped_column("kaisai_nen",primary_key=True)      
    kaisai_tsukihi        :Mapped[str]=mapped_column("kaisai_tsukihi",primary_key=True)      
    keibajo_code          :Mapped[str]=mapped_column("keibajo_code",primary_key=True)      
    kaisai_kai            :Mapped[str]              
    kaisai_nichime        :Mapped[str]              
    race_bango            :Mapped[str]=mapped_column("race_bango",primary_key=True)      
    happyo_tsukihi_jifun  :Mapped[str]              
    toroku_tosu           :Mapped[str]              
    shusso_tosu           :Mapped[str]              
    hatsubai_flag_wide  :Mapped[str]              
    odds_wide           :Mapped[str]              
    hyosu_gokei_wide    :Mapped[str]              

class Umatan(Reflected, Base):
    __tablename__ = "jvd_o4"
    record_id             :Mapped[str]              
    data_kubun            :Mapped[str]              
    data_sakusei_nengappi :Mapped[str]              
    kaisai_nen            :Mapped[str]=mapped_column("kaisai_nen",primary_key=True)      
    kaisai_tsukihi        :Mapped[str]=mapped_column("kaisai_tsukihi",primary_key=True)      
    keibajo_code          :Mapped[str]=mapped_column("keibajo_code",primary_key=True)      
    kaisai_kai            :Mapped[str]              
    kaisai_nichime        :Mapped[str]              
    race_bango            :Mapped[str]=mapped_column("race_bango",primary_key=True)      
    happyo_tsukihi_jifun  :Mapped[str]              
    toroku_tosu           :Mapped[str]              
    shusso_tosu           :Mapped[str]              
    hatsubai_flag_umatan  :Mapped[str]              
    odds_umatan           :Mapped[str]              
    hyosu_gokei_umatan    :Mapped[str]              

class Sanrenpuku(Reflected, Base):
    __tablename__ = "jvd_o5"
    record_id             :Mapped[str]              
    data_kubun            :Mapped[str]              
    data_sakusei_nengappi :Mapped[str]              
    kaisai_nen            :Mapped[str]=mapped_column("kaisai_nen",primary_key=True)      
    kaisai_tsukihi        :Mapped[str]=mapped_column("kaisai_tsukihi",primary_key=True)      
    keibajo_code          :Mapped[str]=mapped_column("keibajo_code",primary_key=True)      
    kaisai_kai            :Mapped[str]              
    kaisai_nichime        :Mapped[str]              
    race_bango            :Mapped[str]=mapped_column("race_bango",primary_key=True)      
    happyo_tsukihi_jifun  :Mapped[str]              
    toroku_tosu           :Mapped[str]              
    shusso_tosu           :Mapped[str]              
    hatsubai_flag_sanrenpuku  :Mapped[str]              
    odds_sanrenpuku           :Mapped[str]              
    hyosu_gokei_sanrenpuku    :Mapped[str]              

class Sanrentan(Reflected, Base):
    __tablename__ = "jvd_o6"
    record_id             :Mapped[str]              
    data_kubun            :Mapped[str]              
    data_sakusei_nengappi :Mapped[str]              
    kaisai_nen            :Mapped[str]=mapped_column("kaisai_nen",primary_key=True)      
    kaisai_tsukihi        :Mapped[str]=mapped_column("kaisai_tsukihi",primary_key=True)      
    keibajo_code          :Mapped[str]=mapped_column("keibajo_code",primary_key=True)      
    kaisai_kai            :Mapped[str]              
    kaisai_nichime        :Mapped[str]              
    race_bango            :Mapped[str]=mapped_column("race_bango",primary_key=True)      
    happyo_tsukihi_jifun  :Mapped[str]              
    toroku_tosu           :Mapped[str]              
    shusso_tosu           :Mapped[str]              
    hatsubai_flag_sanrentan  :Mapped[str]              
    odds_sanrentan           :Mapped[str]              
    hyosu_gokei_sanrentan    :Mapped[str]              


class Track(Base):
    __tablename__="track"
    id:Mapped[uuid.UUID]=mapped_column("id",primary_key=True,default=uuid.uuid4)
    keibajo_code:Mapped[str]#東京競馬場なら"05"
    start_year:Mapped[str]
    end_year:Mapped[str]
    kyori:Mapped[str]
    track_code:Mapped[str]#平地　芝　外回り　など
    babajotai_code:Mapped[str]
    count:Mapped[int]
    mean:Mapped[float]
    std:Mapped[float]
    is_last3f:Mapped[bool]=mapped_column("is_last3f",default=False)

class BackMoney(Base):
    __tablename__="jvd_hr"
    record_id                  :Mapped[str]        
    data_kubun                 :Mapped[str]        
    data_sakusei_nengappi      :Mapped[str]        
    kaisai_nen                 :Mapped[str]=mapped_column("kaisai_nen",primary_key=True)
    kaisai_tsukihi             :Mapped[str]=mapped_column("kaisai_tsukihi",primary_key=True)
    keibajo_code               :Mapped[str]=mapped_column("keibajo_code",primary_key=True)
    kaisai_kai                 :Mapped[str]        
    kaisai_nichime             :Mapped[str]        
    race_bango                 :Mapped[str]=mapped_column("race_bango",primary_key=True)
    toroku_tosu                :Mapped[str]        
    shusso_tosu                :Mapped[str]        
    fuseiritsu_flag_tansho     :Mapped[str]        
    fuseiritsu_flag_fukusho    :Mapped[str]        
    fuseiritsu_flag_wakuren    :Mapped[str]        
    fuseiritsu_flag_umaren     :Mapped[str]        
    fuseiritsu_flag_wide       :Mapped[str]        
    yobi_1                     :Mapped[str]        
    fuseiritsu_flag_umatan     :Mapped[str]        
    fuseiritsu_flag_sanrenpuku :Mapped[str]        
    fuseiritsu_flag_sanrentan  :Mapped[str]        
    tokubarai_flag_tansho      :Mapped[str]        
    tokubarai_flag_fukusho     :Mapped[str]        
    tokubarai_flag_wakuren     :Mapped[str]        
    tokubarai_flag_umaren      :Mapped[str]        
    tokubarai_flag_wide        :Mapped[str]        
    yobi_2                     :Mapped[str]        
    tokubarai_flag_umatan      :Mapped[str]        
    tokubarai_flag_sanrenpuku  :Mapped[str]        
    tokubarai_flag_sanrentan   :Mapped[str]        
    henkan_flag_tansho         :Mapped[str]        
    henkan_flag_fukusho        :Mapped[str]        
    henkan_flag_wakuren        :Mapped[str]        
    henkan_flag_umaren         :Mapped[str]        
    henkan_flag_wide           :Mapped[str]        
    yobi_3                     :Mapped[str]        
    henkan_flag_umatan         :Mapped[str]        
    henkan_flag_sanrenpuku     :Mapped[str]        
    henkan_flag_sanrentan      :Mapped[str]        
    henkan_umaban_joho         :Mapped[str]        
    henkan_wakuban_joho        :Mapped[str]        
    henkan_dowaku_joho         :Mapped[str]        
    haraimodoshi_tansho_1a     :Mapped[str]        
    haraimodoshi_tansho_1b     :Mapped[str]        
    haraimodoshi_tansho_1c     :Mapped[str]        
    haraimodoshi_tansho_2a     :Mapped[str]        
    haraimodoshi_tansho_2b     :Mapped[str]        
    haraimodoshi_tansho_2c     :Mapped[str]        
    haraimodoshi_tansho_3a     :Mapped[str]        
    haraimodoshi_tansho_3b     :Mapped[str]        
    haraimodoshi_tansho_3c     :Mapped[str]        
    haraimodoshi_fukusho_1a    :Mapped[str]        
    haraimodoshi_fukusho_1b    :Mapped[str]        
    haraimodoshi_fukusho_1c    :Mapped[str]        
    haraimodoshi_fukusho_2a    :Mapped[str]        
    haraimodoshi_fukusho_2b    :Mapped[str]        
    haraimodoshi_fukusho_2c    :Mapped[str]        
    haraimodoshi_fukusho_3a    :Mapped[str]        
    haraimodoshi_fukusho_3b    :Mapped[str]        
    haraimodoshi_fukusho_3c    :Mapped[str]        
    haraimodoshi_fukusho_4a    :Mapped[str]        
    haraimodoshi_fukusho_4b    :Mapped[str]        
    haraimodoshi_fukusho_4c    :Mapped[str]        
    haraimodoshi_fukusho_5a    :Mapped[str]        
    haraimodoshi_fukusho_5b    :Mapped[str]        
    haraimodoshi_fukusho_5c    :Mapped[str]        
    haraimodoshi_wakuren_1a    :Mapped[str]        
    haraimodoshi_wakuren_1b    :Mapped[str]        
    haraimodoshi_wakuren_1c    :Mapped[str]        
    haraimodoshi_wakuren_2a    :Mapped[str]        
    haraimodoshi_wakuren_2b    :Mapped[str]        
    haraimodoshi_wakuren_2c    :Mapped[str]        
    haraimodoshi_wakuren_3a    :Mapped[str]        
    haraimodoshi_wakuren_3b    :Mapped[str]        
    haraimodoshi_wakuren_3c    :Mapped[str]        
    haraimodoshi_umaren_1a     :Mapped[str]        
    haraimodoshi_umaren_1b     :Mapped[str]        
    haraimodoshi_umaren_1c     :Mapped[str]        
    haraimodoshi_umaren_2a     :Mapped[str]        
    haraimodoshi_umaren_2b     :Mapped[str]        
    haraimodoshi_umaren_2c     :Mapped[str]        
    haraimodoshi_umaren_3a     :Mapped[str]        
    haraimodoshi_umaren_3b     :Mapped[str]        
    haraimodoshi_umaren_3c     :Mapped[str]        
    haraimodoshi_wide_1a       :Mapped[str]        
    haraimodoshi_wide_1b       :Mapped[str]        
    haraimodoshi_wide_1c       :Mapped[str]        
    haraimodoshi_wide_2a       :Mapped[str]        
    haraimodoshi_wide_2b       :Mapped[str]        
    haraimodoshi_wide_2c       :Mapped[str]        
    haraimodoshi_wide_3a       :Mapped[str]        
    haraimodoshi_wide_3b       :Mapped[str]        
    haraimodoshi_wide_3c       :Mapped[str]        
    haraimodoshi_wide_4a       :Mapped[str]        
    haraimodoshi_wide_4b       :Mapped[str]        
    haraimodoshi_wide_4c       :Mapped[str]        
    haraimodoshi_wide_5a       :Mapped[str]        
    haraimodoshi_wide_5b       :Mapped[str]        
    haraimodoshi_wide_5c       :Mapped[str]        
    haraimodoshi_wide_6a       :Mapped[str]        
    haraimodoshi_wide_6b       :Mapped[str]        
    haraimodoshi_wide_6c       :Mapped[str]        
    haraimodoshi_wide_7a       :Mapped[str]        
    haraimodoshi_wide_7b       :Mapped[str]        
    haraimodoshi_wide_7c       :Mapped[str]        
    yobi_4_1a                  :Mapped[str]        
    yobi_4_1b                  :Mapped[str]        
    yobi_4_1c                  :Mapped[str]        
    yobi_4_2a                  :Mapped[str]        
    yobi_4_2b                  :Mapped[str]        
    yobi_4_2c                  :Mapped[str]        
    yobi_4_3a                  :Mapped[str]        
    yobi_4_3b                  :Mapped[str]        
    yobi_4_3c                  :Mapped[str]        
    haraimodoshi_umatan_1a     :Mapped[str]        
    haraimodoshi_umatan_1b     :Mapped[str]        
    haraimodoshi_umatan_1c     :Mapped[str]        
    haraimodoshi_umatan_2a     :Mapped[str]        
    haraimodoshi_umatan_2b     :Mapped[str]        
    haraimodoshi_umatan_2c     :Mapped[str]        
    haraimodoshi_umatan_3a     :Mapped[str]        
    haraimodoshi_umatan_3b     :Mapped[str]        
    haraimodoshi_umatan_3c     :Mapped[str]        
    haraimodoshi_umatan_4a     :Mapped[str]        
    haraimodoshi_umatan_4b     :Mapped[str]        
    haraimodoshi_umatan_4c     :Mapped[str]        
    haraimodoshi_umatan_5a     :Mapped[str]        
    haraimodoshi_umatan_5b     :Mapped[str]        
    haraimodoshi_umatan_5c     :Mapped[str]        
    haraimodoshi_umatan_6a     :Mapped[str]        
    haraimodoshi_umatan_6b     :Mapped[str]        
    haraimodoshi_umatan_6c     :Mapped[str]        
    haraimodoshi_sanrenpuku_1a :Mapped[str]        
    haraimodoshi_sanrenpuku_1b :Mapped[str]        
    haraimodoshi_sanrenpuku_1c :Mapped[str]        
    haraimodoshi_sanrenpuku_2a :Mapped[str]        
    haraimodoshi_sanrenpuku_2b :Mapped[str]        
    haraimodoshi_sanrenpuku_2c :Mapped[str]        
    haraimodoshi_sanrenpuku_3a :Mapped[str]        
    haraimodoshi_sanrenpuku_3b :Mapped[str]        
    haraimodoshi_sanrenpuku_3c :Mapped[str]        
    haraimodoshi_sanrentan_1a  :Mapped[str]        
    haraimodoshi_sanrentan_1b  :Mapped[str]        
    haraimodoshi_sanrentan_1c  :Mapped[str]        
    haraimodoshi_sanrentan_2a  :Mapped[str]        
    haraimodoshi_sanrentan_2b  :Mapped[str]        
    haraimodoshi_sanrentan_2c  :Mapped[str]        
    haraimodoshi_sanrentan_3a  :Mapped[str]        
    haraimodoshi_sanrentan_3b  :Mapped[str]        
    haraimodoshi_sanrentan_3c  :Mapped[str]        
    haraimodoshi_sanrentan_4a  :Mapped[str]        
    haraimodoshi_sanrentan_4b  :Mapped[str]        
    haraimodoshi_sanrentan_4c  :Mapped[str]        
    haraimodoshi_sanrentan_5a  :Mapped[str]        
    haraimodoshi_sanrentan_5b  :Mapped[str]        
    haraimodoshi_sanrentan_5c  :Mapped[str]        
    haraimodoshi_sanrentan_6a  :Mapped[str]        
    haraimodoshi_sanrentan_6b  :Mapped[str]        
    haraimodoshi_sanrentan_6c  :Mapped[str]       
    
Reflected.prepare(engine)
