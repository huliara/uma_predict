...
def shutsubahyo(hist_number: int, race: Race, db: Session):
    horses = db.scalars(
        select(Career)
        .filter(
    ...
    """着差でスムージングした正解ラベルを計算"""
    chakusa_list = ["   "] * 17
    nyusen_umaban_list = [int(horse.umaban) for horse in horses]
    for horse in horses:
    ...
    for horse in horses:
        ...
        for hist in horse_hist:
            ...
            hist_dict = {
                ...
                "kyoso_joken_code": kyoso_joken_mapper(
                    past_race.kyoso_joken_code
                ),
