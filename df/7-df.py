...
for race in races:
    ...
    pprint.pprint(race.kaisai_tsukihi)
    horses = db.scalars(
        select(Career)
        .filter(
...
pprint.pprint(final_df)
print(final_df.null_count().sum(axis=1))
time_end = time.time()
print(time_end - parse_start)
target_year = "2022"
final_df.write_parquet(f"data7/{target_year}-{hist_number}-{repeat_number}.parquet")
